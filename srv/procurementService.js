...
function mapAllFields(data) {
    return {
        ...
        wbsElement: data.WBSElement || "NO WBS",
        // Organization
...
module.exports = cds.service.impl(async function () {
    ...
    this.on('UpsertSalesContractData', async (req) => {
        ...
        try {
            ...
            const allWBS = [
                ...new Set(
                    items.map(i => i.WBSElement).filter(Boolean)
                )
            ];
            ...
            if (allWBS.length > 0) {
                const wbsData = await s4hprojects.run(
                    SELECT.from('WBSElement')
                        .columns('WBSElementInternalID', 'WBSElementExternalID')
                        .where({ WBSElementExternalID: { in: allWBS } })
                );

                wbsData.forEach(w => {
                    wbsMap[w.WBSElementExternalID] = w.WBSElementInternalID;
                });
            }

            console.log("WBS Mapping (External → Internal):");
            console.log(wbsMap);
            ...
            for (const header of headers) {
                ...
                console.log(`\n Processing Contract: ${header.SalesContract}`);

                const contractItems = itemsByContract[header.SalesContract] || [];
                
                const headerWBS = contractItems
                    .map(i => wbsMap[i.WBSElement])
                    .filter(Boolean)[0] || "NO WBS";

                const headerWithWBS = {
                    ...header,
                    WBSElement: headerWBS
                };

                const mappedHeader = mapAllFields(headerWithWBS);

                allHeaders.push(mappedHeader);
               
                const mappedItems = contractItems.map(item => {

                    const itemWithWBS = {
                        ...item,
                        WBSElement: wbsMap[item.WBSElement] || "NO WBS"
                    };

                    const mappedItem = mapSalesOrderItem(itemWithWBS);
            ...
            if (allHeaders.length > 0) {
                await tx.run(
                    UPSERT.into('SalesOrderHeader').entries(allHeaders)
                );
            }
            ...
            if (allItems.length > 0) {
                await tx.run(
                    UPSERT.into('SalesOrderItem').entries(allItems)
                );
            }
